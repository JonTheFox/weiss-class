...
export default function Signup(props) {
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      if (!data) throw new Error("Did not receive any data from the server");
      ...
      >
        ...
        {fields.map(
          (
            ...
            if (type === "imageUpload") {
              return (
                <Grid key={label} className={classes.fileUploaderGrid}>
                  <ImageUploader
                    withIcon={false}
                    label={"Accepted types: jpeg, jpg and png"}
              ...
              return (
                <Grid item xs={10} sm={10} key={label}>
                  <Autocomplete
                    ...
                    loading={false}
                    loadingText={"Working.."}
                    noOptionsText={"Imagine there's no countries.."}
                    renderInput={(params) => (
                      <React.Fragment>
                        <div
                          className={clsx("search-icon", classes.searchIcon)}
                        >
                          <SearchIcon />
                        </div>
            ...
            const renderTextField = () => {
              return (
                <TextField
                  ...
                  select={type === "select"}
                  helperText={name === "password" && PASSWORD_REQUIREMENTS}
                  type={type}
                  required={required}
                  id={name}
            ...
            return (
              <Grid
                item
                xs={isNarrowField ? 4 : isMediumSizeField ? 10 : 12}
                sm={isNarrowField ? 4 : isMediumSizeField ? 10 : 12}
                key={label}
              >
                {renderTextField()}
              </Grid>
  ...
  const FeedbackMessage = ({ heading, bodyText, btnText = "", onBtnClick }) => {
      ...
      <main className={classes.layout}>
        <Paper className={classes.paper}>
          ...
          <Stepper
            ...
            className={clsx(
              classes.stepper,
              showFeedback && feedback.type === "error" && classes.error
            )}
          >
            {FORMS.map(({ label }) => (
              <Step key={label}>
