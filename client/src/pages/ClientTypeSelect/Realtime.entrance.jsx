  ...
  useEffect(() => {
    ...
    switch (connectionStatus) {
      ...
      case ALREADY_INSIDE_ROOM:
        promiseKeeper.reject(CONNECTION_TIMEOUT_LABEL, "connection is active.");
        newFeedback = "We're already inside :) ";
        promiseKeeper.stall(DURATIONS.enter * 3, "hide pie chart").then(() => {
      ...
      case ENTERED_ROOM:
        ...
        setShowGlass(true);
        promiseKeeper.stall(DURATIONS.enter * 1, "hide PieChart").then(() => {
          setShowPieChart(false);
          promiseKeeper.stall(1 * 1000, "navigate to classroom").then(() => {
            //debugger;
    ...
    >
      ...
      <div className={clsx("section section--header flex")}>
        <Container maxWidth="sm">
          <ScaleIn initialPose="exit" pose={hideElements ? "exit" : "enter"}>
      ...
      <div className={clsx("section section--pie")}>
        <PosedPieContainer
          className={clsx(
            ...
            [ENTERED_ROOM, ALREADY_INSIDE_ROOM].includes(connectionStatus) &&
              "pause-children"
          )}
        ...
        >
          <PieChart
            ...
            showHoverAnimation={Boolean([IDLE].includes(connectionStatus))}
            data={pieData}
            onClick={({ chartData, sectionIndex, title }) => {
              ...
              if (!user) {
                loggError("Cannot connect to socket without being logged in.");
              ...
              promiseKeeper.stall(1000 * 0.2, "hide glass").andThen(() => {
                setShowGlass(false);

                promiseKeeper.stall(1 * 1000, "hide pie chart").then(() => {
                  setShowPieChart(false);
