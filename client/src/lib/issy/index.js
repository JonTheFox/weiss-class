  ...
  class Validator {
    ...
    _is = (aType = "") => {
      ...
      switch (aType) {
        ...
        case "anArray":
          result =
            Object.prototype.toString.call(this.arg) === "[object Array]";
          break;
        case "anObject":
          result =
            arg !== null &&
        ...
        case "aDate":
          result =
            Object.prototype.toString.call(this.arg) === "[object Date]" &&
            !isNaN(this.arg.getTime())
              ? true
              : false;
          break;
        case "aValidDate":
        ...
        default:
          throw new Error("Unrecognized data type argument provided to issy");
      }
...
var timing = (function () {
  ...
  const _isTimeout = (str) =>
    str && str.length && ["timeout", "timeouts"].includes(str.toLowerCase());
  let timeoutIndex = 0;
  let intervalIndex = 0;
  const _clear = (timeoutOrInterval = "timeout", timerID, timerItem = {}) => {
    ...
    try {
      isTimeout ? window.clearTimeout(timerID) : window.clearInterval(timerID);
      return true;
    } catch (err) {
      return null;
    }
  ...
  class PromiseKeeper {
    ...
    _addOrUpdateUltimatePromise = (ultimatePromise) => {
      ...
      if (!label) {
        loggError("_addOrUpdateUltimatePromise() was called without a label");
        return null;
      }
    ...
    stall = function (ms = 50, label, config) {
      ...
      label =
        ...
        `${this.label ? this.label + " - " : ""}stall (timeout${timeoutIndex})`;
    ...
    every = (ms = 1000, callback, label) => {
      if (!callback) {
        loggError(`every() was called without a callback. Ignoring call. `);
        return null;
      }
      intervalIndex++;
      ...
      try {
        ...
        switch (sanitizedLabel) {
          case "":
            ...
            this.latestPromise = {};
            if (removeFromCache) {
              const { sanitizedLabel: _sanitizedLabel } = timerElement;
              delete this.promises[_sanitizedLabel];
            }

          case "all":
            for (let [key, _timerElement] of Object.entries(this.promises)) {
              if (_timerElement.id) {
                _clear(timerType, _timerElement.id, _timerElement);
          ...
          default:
            ...
            if (!timerElement) {
              logg(`clear(): Did not find a promise with label ${label}`);
              return false;
            }
            if (!timerElement.id) {
              logg(
        ...
        logg(this.logLabel + `Could not clear ${timerType} "${label}"`, err);
        return null;
      }
    };
    ...
    withRC = function (originalPromise, config = {}) {
      ...
      const ultimatePromise = new Promise((ultimateResolve, ultimateReject) => {
      ...
      originalPromise.catch((reason = "") => {
        try {
          ...
          if (noRejection) {
            ...
            logg(
              logMsg + "--> REJECTION IGNORED! Resolving instead. ",
              ultimatePromise
            );
            return _ultimateResolve(null, reasonString);
            // return true;
    ...
    reject = (promiseLabel, reason = "unspecified") => {
      try {
        if (!promiseLabel) {
          loggError(`Can't reject a promise without a specified label`);
          return null;
        }
        ...
        if (!promiseToReject) {
          logg(`Did not find a promise with a label of "${promiseLabel}")`);
          return null;
        }
    ...
    resolve = (promiseLabel, resolveValue) => {
      try {
        if (!promiseLabel) {
          loggError(`Can't resolve a promise without a specified label`);
          return null;
        }
    ...
    completeAll = (resolveOrReject = "reject", valOrReason = "") => {
      ...
      const completefunc = resolveOrReject === "resolve" ? "resolve" : "reject";
      ...
      for (let [key, _promise] of Object.entries(_promises)) {
        ...
        logg(
          `${this.logLabel || ""} ${reason} of promise "${_promise.label}". `,
      ...
      const finalUltimatePromise = this.withRC(
        new Promise(async (ultimateResolve, ultimateReject) => {
          ...
          const results = softPromises.map((softPromise, j) => {
            const { originalPromise, label = "no_label" } = softPromise;

            const { resolved, rejected, completed, result } = originalPromise;
  ...
  return new Promise((resolve, reject) => {
    ...
    image.onload = (ev) => {
      if (!image.decode) {
        ...
        logg("Image.decode() is not supported in browser (older Firefox?)");
...
const getUniqueString = (numChars = 10) => {
  let rand = Math.random().toString(36).slice(2, numChars);
  return "_" + rand;
};
      ...
      do {
        ...
        if (attr) {
          ...
          for (let [attrName, attrValue] of Object.entries(attr)) {
            const ancestorAttrValue = ancestor.getAttribute(attrName);
...
const alignElements = ({ movingElem = {}, targetElem = {}, top = 0 }) => {
  try {
    ...
    if (!_movingElem) throw new Error(`Missing or bad argument: movingElem`);
    const _targetElem = getCurrentFromRef(targetElem);
    if (!_targetElem) throw new Error(`Missing or bad argument: targetElem`);
...
const setDomProperty = (config = {}) => {
  try {
    if (!is(config).anObject) {
      throw new Error("setDomProperty was called without a config object");
    }
    const {
