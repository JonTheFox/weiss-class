...
var timing = (function () {
  ...
  class PromiseKeeper {
    ...
    _addOrUpdate = (timeoutOrInterval = "timeout", id, ms = 0, label = "") => {
    ...
    clear = (timeoutOrInterval = "timeout", label = "") => {
      ...
      const timerType = _isTimeout(timeoutOrInterval) ? "timeout" : "interval";
      try {
        ...
        loggError(
          this.logLabel +
            `Could not clear ${_isTimeout ? "timeout" : "interval"} "${label}"`,
          err
        );
        return null;
      }
...
const climbFrom = (domElem) => {
  debugger;
  if (!domElem || !(domElem instanceof HTMLElement)) return new DOMClimber({});
...
const alignElements = ({
  ...
  if (["top", "bottom"].includes(offsetProp)) {
    ...
    if (
      ...
      if (["up", "left"].includes(push)) relativePadding = -1 * relativePadding;
    }

    newPropVal = targetBox[offsetProp] + extraPadding + relativePadding + "px";
