...
const { logger, request, sanitizeVarName, getRandomUpTo, is, getUniqueString } =
  global;
...
class ClassroomClient {
  ...
  img = {};
  constructor(config = {}) {
    const { email, first_name, last_name, clientId, userType, roomKey, img } =
      config;
...
class Classroom {
  ...
  constructor(config = {}) {
    ...
    const getTeacherFullName = (firstTeacher = {}) => {
      ...
      const _first_name = first_name[0].toUpperCase() + first_name.slice(1);
      const _last_name =
        last_name &&
        last_name[0] &&
  ...
  hasTeacher = ({ email } = {}) => {
    ...
    try {
      if (!email) {
        throw new Error(`Can't find a teacher without its email address`);
      }
  ...
  getInfo = (exclude = {}) => {
    const { teachers, students, platforms, roomKey, name, title, index } = this;
  ...
  classroomsIO.on("connection", function (socket) {
    ...
    socket.on(
      ...
      ({ clientId, roomKey, clientType = "" }) => {
        ...
        return classroomsIO.to(roomKey).emit("server__admitsAnotherUser", {
          ...userWithoutPass,
        });
      }
    );
    ...
    socket.on(
      "client__providesCredentials",
      async function ({ user, clientType = "", clientId, roomKey }) {
        try {
          ...
          const clientTypeMsg = `${clientType} ${
    ...
    socket.on("client__teacherSendsAction", (payload) => {
      ...
      try {
        ...
        if (recipients == "allStudents") {
          ...
          return socket.to(roomKey).emit("server__teacherSendsAction", {
            roomKey,
            teacher: teacherInfo,
            actionName,
            bodyText,
    ...
    socket.on("client__createRoom", function (payload) {
      try {
        ...
        if (!intent) {
          throw new Error(`user_createRoom was sent without an intent`);
        }
